...
PM spawns `engineer` to execute kanban cards — one card per spawn, isolated child process on Sonnet 4.5. Inner skills it auto-discovers under `.pi/skills/`: `frontend`, `backend`, `db-mysql`, `db-postgres`, `uiux`, `devops`, `debugger`, `refactor`, `tdd`, `improve-codebase-architecture`, `zoom-out`, `prototype`, `review-artifact`, `corpus-learning`, `planning`, `research`, `commit-and-push`. After build it spawns `uat-tester` (user-facing features) and `red-team` (anything touching auth, user input, or external network I/O); their findings land on the card body, not in chat.
...
Six skills from [mattpocock/skills](https://github.com/mattpocock/skills) ("skills for real engineers") are imported under `.pi/skills/` and wired into PM and engineer. Each carries a `## Source` footer linking back to Matt's repo; `AGENTS.md` carries the full provenance audit.

| Skill | Caller | What it does |
|---|---|---|
| `grill-me` | pm | Relentlessly interview the user about a fuzzy plan, one question at a time, until every branch of the decision tree resolves. Use before `prd` when the problem is too soft to draft against. |
| `grill-with-docs` | pm | Same grilling discipline, but maintains the project's `## Glossary` section in `project.md` and per-project ADRs (`<vault>/projects/<slug>/adr/<NNNN>-slug.md`) inline as decisions crystallise. **Option-3 hybrid**: glossary lives next to project goals/stakeholders; ADRs stay separate, one file per decision. |
| `zoom-out` | pm + engineer | One-liner: *"go up a layer of abstraction; give me a map of all relevant modules using the project's glossary vocabulary."* Use at the start of work in unfamiliar code. |
| `tdd` | engineer | Red-green-refactor with strict vertical-slice discipline — forbids horizontal slicing ("write all tests, then all code"). Tests target observable behaviour through public interfaces, not implementation. Six sidecar files (`tests.md`, `mocking.md`, `deep-modules.md`, `interface-design.md`, `refactoring.md`) carry the progressive-disclosure depth. |
| `improve-codebase-architecture` | engineer | Surface architectural friction and propose **deepening opportunities** — turn shallow modules into deep ones using a fixed vocabulary (depth, seam, adapter, leverage, locality, deletion test). Outputs an editorial HTML report (Tailwind + Mermaid, before/after diagrams) to `$TMPDIR`. **Option-3 hybrid** for inputs (reads `project.md` glossary + `adr/`); explicit preference for the local CodeGraph MCP tools during exploration. Optional vault-archive step routes findings through `note-taker` to `projects/<slug>/architecture-reviews/`. |
| `prototype` | pm (UI branch) + engineer (LOGIC branch) | Build throwaway code that answers one design question. **LOGIC** = tiny interactive terminal app that drives a state machine by hand (engineer reaches for this when a card's state model looks fine on paper but unclear under edge cases). **UI** = 3 radically different layouts on a real route via `?variant=` with a floating switcher (PM reaches for this *upstream* of `designer`, for "what shape" questions). Pure logic module stays portable; TUI/UI shells get deleted once the question is answered. |

The existing [`debugger`](.pi/skills/debugger/SKILL.md) skill also **absorbed** Matt's `diagnose` feedback-loop discipline: 10 feedback-loop construction techniques, "treat the loop as a product" iteration, non-deterministic-bug handling, 3–5 ranked-falsifiable hypotheses (replacing the previous one-hypothesis-at-a-time approach), tagged `[DEBUG-<hex>]` instrumentation, the correct-seam regression-test insight, and a Phase-6 hand-off to `improve-codebase-architecture`. Original hypothesis-loop framing, output template, and caller notes (engineer / trader) preserved.

The two option-3 adaptations (`grill-with-docs`, `improve-codebase-architecture`) are deliberate: this project's domain language belongs in `project.md` next to goals/stakeholders/handover (one orientation file per project), while ADRs stay separate so each decision is link-stable and the trail is append-only. See `AGENTS.md` § *Imported skills (provenance)* for the full audit table.
...
**Why everything user-facing lives under the vault.** The vault is the only directory that needs to sync between devices — point one sync tool at it and notes, artifacts, and memory all travel together. Currently that's **iCloud Drive** (the vault is an iCloud-backed Obsidian vault, so opening Pi on a second Mac picks up the same notes, last week's PDFs, and the latest persona profiles with zero extra config). If iCloud's quirks become limiting — large-PDF storage costs, conflict semantics, conflicted-copy churn under concurrent edits — the same layout works under **[Syncthing](https://syncthing.net)** without code changes: point Syncthing at the vault root, set `AGENTS_TEAM_VAULT_PATH` to that path on each device, done. Sessions, the per-run research tree, and the SRS deck deliberately stay in `.pi/state/` (not synced) because they're operational state where a multi-device merge would be meaningless or destructive.

The artifact and memory env vars accept absolute paths and win over the vault-relative defaults — useful when you want to opt one layer *out* of sync (e.g. keep large PDFs off iCloud) while keeping everything else synced through the vault.
