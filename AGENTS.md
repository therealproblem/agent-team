...
| ext | terminal-image | Functional. Auto-detects terminal/TUI prompts that include local JPEG/PNG/WebP/GIF paths plus visual intent, validates file existence/type/10 MB cap, base64-encodes to `ImageContent`, and sends the original prompt text plus images via `pi.sendUserMessage`. File-operation prompts (move/copy/delete/archive/etc.) remain plain text. Text-only active models get a clear limitation message. |
