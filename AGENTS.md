...
| ext | terminal-image | Functional. Registers `/image <path> <question>` for explicit terminal/TUI local image attachment. Reads the local file, validates JPEG/PNG/WebP/GIF by magic bytes or extension, caps size at 10 MB, base64-encodes to `ImageContent`, and sends it with the question via `pi.sendUserMessage`. Text-only active models get a clear limitation message; normal chat path mentions stay plain text. |
