...
Save the full synthesis as markdown via `note-taker` at `pm/user-research/<date>-<slug>.md` — Obsidian's graph will surface backlinks from the PRD that cites it. If the synthesis would benefit from an interactive read (per-participant tabs, problem-frequency sparklines, side-by-side quote grids), follow up with `render-html` to publish HTML. The chat reply is the vault path + a one-line summary of the top problem identified.
