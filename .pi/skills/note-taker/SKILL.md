...
Cases where rendering is worth it:
...
8. **Return** `{ path, title, vault_relative_path }` to the caller. The vault path is what `render` needs if a follow-up render is wanted.
...
If the caller (persona) wants to also render to HTML, they call `render` next. They do not bundle the render URL into the same reply unless render has actually run.

## Don't

- **Don't write HTML to the vault.** Obsidian's graph doesn't index it; it breaks the conventions. HTML output is the `render` skill's job, and it writes outside the vault.
