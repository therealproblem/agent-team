...
- **Manual refresh: `/news-refresh`.** Runs the same code path as `refresh_all_topics`, entirely inside the extension — no agent turn, no LLM cost. Use any time you want to force a re-pull (e.g. if the morning cron skipped because the laptop was asleep). Surfaces a "started" line immediately and a "done" line when the HTTP sweep completes (typically 5–30s).
- **Refresh errors are logged.** Every refresh (cron, `/news-refresh`, `refresh_all_topics`) writes a snapshot of feed errors to `.pi/state/news-errors.json`. Schema: `{ last_refresh_at, trigger, topics_attempted, entries: [{ topic, feed_url, message, type }] }` where `type` is one of `http_4xx | http_5xx | http_other | timeout | network | parse | other`. When the user asks "what was the error", "why is news empty", "did the morning refresh fail", or similar, `read('.pi/state/news-errors.json')` and summarize: trigger + timestamp, how many feeds failed, grouped by error type (e.g. "3 feeds returned 403, 1 timeout"), and which topics were affected. Empty `entries` = last refresh was clean.
