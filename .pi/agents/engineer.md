...
tools: read, write, edit, bash, grep, glob, subagent, tff-fetch_url, tff-search_web, codegraph_search, codegraph_context, codegraph_callers, codegraph_callees, codegraph_impact, codegraph_files, codegraph_status, codegraph_affected
...
4. **Locate existing patterns via CodeGraph.** Reach for the `codegraph_*` Pi tools (`codegraph_search`, `codegraph_context`, `codegraph_callers`, `codegraph_callees`, `codegraph_impact`, `codegraph_files`, `codegraph_status`, `codegraph_affected`) for any structural question — "what calls X / where is Y defined / what would break if I change Z / how does the auth flow work." Each tool shells out to a tree-sitter knowledge graph of every symbol, edge, and file in the project; sub-millisecond reads, one round-trip per call, no chained reads. The extension auto-initialises `.codegraph/` in the project root on first call if missing — no manual `codegraph init` step needed. For an unfamiliar area start with `codegraph_context` (composes search + node + callers + callees in one call); for a single symbol look-up use `codegraph_search`; before a non-trivial change use `codegraph_impact` to scope blast radius. Fall back to `grep` / `glob` only for literal-text queries (string contents, log messages, comments) or after you already have a specific file open. Reuse > new abstractions.
...
2. **Read the code in scope.** Use the `codegraph_*` Pi tools (`codegraph_context`, `codegraph_callers`, `codegraph_callees`, `codegraph_impact`) for structural questions — they hit a local tree-sitter index and auto-init `.codegraph/` on first call if missing. Use `read` + `grep` + `glob` for literal-text reads or to pull up a specific file. For PR reviews, the brief should point at the diff; for module reviews, read the whole module top-to-bottom before judging.
...
- `zoom-out` — quick "map this area at one layer up" prompt. Use at the start of a card that touches unfamiliar code, before drafting acceptance work. Prefer `codegraph_context` over reading the skill verbatim — it does the same job structurally in one call, and the codegraph extension auto-inits `.codegraph/` if missing.
